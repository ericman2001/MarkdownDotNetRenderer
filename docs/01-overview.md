...
| Outputs | Self-contained HTML (inline SVG), ODT (native SVG pictures) and DOCX (embedded SVG) — all implemented ([phase 1](phases/phase-1-html-flowchart.md), [phase 2](phases/phase-2-odf-output.md), [phase 5](phases/phase-5-docx.md)) |
| Platforms | Linux, macOS, Windows — all first-class; no Windows-only APIs, no Office automation |
| DOCX diagrams | **SVG-only** embedding (Word 2016+, and LibreOffice Writer displays it too). PNG raster fallback **deferred** |
...
| DOCX writer | DocumentFormat.OpenXml (MIT, pinned `3.5.1`), isolated behind `IDocumentWriter`; measured AOT-clean ([06](06-aot-and-dependencies.md)) |
